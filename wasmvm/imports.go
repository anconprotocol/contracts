...
func (d *GoInstance) GetExports() *wasmer.Exports {
	return d.inst.Exports
}
...
func NewInstance(b []byte, s *sdk.Storage) (*GoInstance, error) {
	c := wasmer.NewConfig().UseSinglepassCompiler()
	engine := wasmer.NewEngineWithConfig(c)
...
//	importObject.Register("env", sdkContractRuntime(store, data, s))
...
func sdkContractRuntime(store *wasmer.Store, data *GoInstance, s *sdk.Storage) map[string]wasmer.IntoExtern {
	...
	return map[string]wasmer.IntoExtern{
		...
		"dagJsonPut": wasmer.NewFunction(
			store,
			wasmer.NewFunctionType(wasmer.NewValueTypes(wasmer.I32), wasmer.NewValueTypes()),
			func(args []wasmer.Value) ([]wasmer.Value, error) {
				// sp := args[0].Unwrap()
				// n, err := sdk.Decode(basicnode.Prototype.Any, sp.(string))
				// if err != nil {
				// 	return nil, err
				// }
				// // link := s.Store(linking.LinkContext{}, n)
				// //v := js.ValueOf(link.String())
